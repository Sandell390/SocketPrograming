...
namespace Shared
{
    abstract public class Protocol<TMessageType>
    {
        ...
        async Task<TMessageType> ReadBody(NetworkStream networkStream, int bodyLenght) 
        {
            var bodyBytes = await ReadAsync(networkStream,bodyLenght).ConfigureAwait(false);
        ...
        protected void AssertValidMessageLength(int messageLength)
        {
            if (messageLength < 1)
                throw new ArgumentOutOfRangeException("Invaild Message Length");
        }
    }
